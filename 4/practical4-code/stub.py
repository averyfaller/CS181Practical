...
#        return 0
    ...
    def state_action_to_array(self, state, action, prev_state):
        if state is not None and prev_state is not None:
            ...
            arr = [(state['monkey']['top'] - 28), 
                   (state['tree']['top'] - 100) - (state['monkey']['top'] - 28), 
                   (state['tree']['top'] - 100) - (state['monkey']['top'] - 28)^2,
                   state['tree']['dist'], state['monkey']['vel'],
                   prev_state['monkey']['vel'] - state['monkey']['vel'], action * 1]
    ...
    def Q_to_string(self, state, action):
        bin_size = 2
        if state == None:
            return None
        
        q_string = str(state['tree']['bot']//bin_size) + '_' + \
            str(state['tree']['top']//bin_size) + '_' + \
            str(state['tree']['dist']//bin_size) + '_' + \
            str(state['monkey']['bot']//bin_size) + '_' + \
            str(state['monkey']['top']//bin_size) + '_' + \
            str(state['monkey']['vel']) + '_' + \
            str(action)
    ...
    def set_Q_score(self, state, action, q, prev_state):
        ...
        arr = self.state_action_to_array(state, action, prev_state)
        if arr is not None:
            self.X.append(arr)
            self.y.append(q)
    ...
    def action_callback(self, state):
        ...
        self.set_Q_score(self.last_state, new_action, best_Q, self.penultimate_state)
